...
function initSchema(db) {
  ...
  // Migration: Add execution_mode if not exists
  try {
    db.prepare("ALTER TABLE campaigns ADD COLUMN execution_mode TEXT DEFAULT 'polling' CHECK(execution_mode IN ('polling', 'webhook'))").run();
  } catch (e) {
...
export function createCampaign(data) {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (@name, @ig_media_id, @ig_media_url, @ig_media_caption, @trigger_type, @keywords, @check_follower, @dm_default, @dm_follower, @dm_non_follower, @is_active, @execution_mode)
  `);
