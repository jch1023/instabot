...
async function initSchema() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS campaigns (
      ...
      reply_text_1 TEXT DEFAULT '',
      reply_active_1 INTEGER DEFAULT 0,
      reply_text_2 TEXT DEFAULT '',
      reply_active_2 INTEGER DEFAULT 0,
      reply_text_3 TEXT DEFAULT '',
      reply_active_3 INTEGER DEFAULT 0,
      reply_text_4 TEXT DEFAULT '',
      reply_active_4 INTEGER DEFAULT 0,
      reply_text_5 TEXT DEFAULT '',
      reply_active_5 INTEGER DEFAULT 0,
      reply_rotation_index INTEGER DEFAULT 0,
  ...
  // Reply (대댓글) columns
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_enabled INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_text_1 TEXT DEFAULT ''`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_active_1 INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_text_2 TEXT DEFAULT ''`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_active_2 INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_text_3 TEXT DEFAULT ''`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_active_3 INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_text_4 TEXT DEFAULT ''`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_active_4 INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_text_5 TEXT DEFAULT ''`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_active_5 INTEGER DEFAULT 0`;
  await sql`ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS reply_rotation_index INTEGER DEFAULT 0`;
...
export async function createCampaign(data) {
  await ensureSchema();
  const hasLegacyCtaButtonText = Object.prototype.hasOwnProperty.call(data, 'cta_button_text');
  const hasFollowerCtaButtonText = Object.prototype.hasOwnProperty.call(data, 'cta_follower_button_text');
  const hasNonFollowerCtaButtonText = Object.prototype.hasOwnProperty.call(data, 'cta_non_follower_button_text');
  const { rows } = await sql`
    INSERT INTO campaigns (
      ...
      reply_enabled, reply_text_1, reply_active_1, reply_text_2, reply_active_2,
      reply_text_3, reply_active_3, reply_text_4, reply_active_4, reply_text_5, reply_active_5,
    ...
    VALUES (
      ...
      ${hasLegacyCtaButtonText ? (data.cta_button_text ?? '') : '팔로우 했어요'},
      ${data.cta_payload || 'FOLLOW_RECHECK'},
      ${data.cta_follower_enabled ? 1 : 0},
      ${hasFollowerCtaButtonText ? (data.cta_follower_button_text ?? '') : '팔로워 확인했어요'},
      ${data.cta_follower_payload || 'FOLLOWER_RECHECK'},
      ${Object.prototype.hasOwnProperty.call(data, 'cta_follower_prompt')
      ? (data.cta_follower_prompt ?? '')
      : '아래 버튼을 눌러 진행해주세요.'},
      ${data.cta_non_follower_enabled !== false ? 1 : 0},
      ${hasNonFollowerCtaButtonText ? (data.cta_non_follower_button_text ?? '') : '팔로우 했어요'},
      ${data.cta_non_follower_payload || 'FOLLOW_RECHECK'},
      ${Object.prototype.hasOwnProperty.call(data, 'cta_non_follower_prompt')
      ? (data.cta_non_follower_prompt ?? '')
      : '아래 버튼을 눌러 팔로우 상태를 다시 확인해주세요.'},
      ${data.reply_enabled ? 1 : 0},
      ${data.reply_text_1 || ''},
      ${data.reply_active_1 ? 1 : 0},
      ${data.reply_text_2 || ''},
      ${data.reply_active_2 ? 1 : 0},
      ${data.reply_text_3 || ''},
      ${data.reply_active_3 ? 1 : 0},
      ${data.reply_text_4 || ''},
      ${data.reply_active_4 ? 1 : 0},
      ${data.reply_text_5 || ''},
      ${data.reply_active_5 ? 1 : 0},
...
export async function updateCampaign(id, data) {
  ...
  await sql`
    UPDATE campaigns SET
      ...
      cta_button_text = ${hasLegacyCtaButtonText ? (data.cta_button_text ?? '') : '팔로우 했어요'},
      cta_payload = ${data.cta_payload || 'FOLLOW_RECHECK'},
      cta_follower_enabled = ${data.cta_follower_enabled ? 1 : 0},
      cta_follower_button_text = ${hasFollowerCtaButtonText ? (data.cta_follower_button_text ?? '') : '팔로워 확인했어요'},
      cta_follower_payload = ${data.cta_follower_payload || 'FOLLOWER_RECHECK'},
      cta_follower_prompt = ${Object.prototype.hasOwnProperty.call(data, 'cta_follower_prompt')
      ? (data.cta_follower_prompt ?? '')
      : '아래 버튼을 눌러 진행해주세요.'},
      cta_non_follower_enabled = ${data.cta_non_follower_enabled !== false ? 1 : 0},
      cta_non_follower_button_text = ${hasNonFollowerCtaButtonText ? (data.cta_non_follower_button_text ?? '') : '팔로우 했어요'},
      cta_non_follower_payload = ${data.cta_non_follower_payload || 'FOLLOW_RECHECK'},
      cta_non_follower_prompt = ${Object.prototype.hasOwnProperty.call(data, 'cta_non_follower_prompt')
      ? (data.cta_non_follower_prompt ?? '')
      : '아래 버튼을 눌러 팔로우 상태를 다시 확인해주세요.'},
      reply_enabled = ${data.reply_enabled ? 1 : 0},
      reply_text_1 = ${data.reply_text_1 || ''},
      reply_active_1 = ${data.reply_active_1 ? 1 : 0},
      reply_text_2 = ${data.reply_text_2 || ''},
      reply_active_2 = ${data.reply_active_2 ? 1 : 0},
      reply_text_3 = ${data.reply_text_3 || ''},
      reply_active_3 = ${data.reply_active_3 ? 1 : 0},
      reply_text_4 = ${data.reply_text_4 || ''},
      reply_active_4 = ${data.reply_active_4 ? 1 : 0},
      reply_text_5 = ${data.reply_text_5 || ''},
      reply_active_5 = ${data.reply_active_5 ? 1 : 0},
...
export async function updateReplyRotationIndex(campaignId, newIndex) {
  await ensureSchema();
  await sql`UPDATE campaigns SET reply_rotation_index = ${newIndex} WHERE id = ${campaignId}`;
}
...
export async function upsertPendingFollowRecheck(accountId, data) {
  ...
  const hasCtaButtonText = Object.prototype.hasOwnProperty.call(data, 'ctaButtonText');
  const hasCtaPayload = Object.prototype.hasOwnProperty.call(data, 'ctaPayload');
  const ctaButtonTextValue = hasCtaButtonText ? (data.ctaButtonText ?? null) : null;
  const ctaPayloadValue = hasCtaPayload ? (data.ctaPayload ?? null) : null;
  ...
  await sql`
    ...
    DO UPDATE SET
      ...
      cta_button_text = CASE
        WHEN ${hasCtaButtonText ? 1 : 0} = 1 THEN ${ctaButtonTextValue}
        ELSE follow_recheck_pending.cta_button_text
      END,
      cta_payload = CASE
        WHEN ${hasCtaPayload ? 1 : 0} = 1 THEN ${ctaPayloadValue}
        ELSE follow_recheck_pending.cta_payload
      END,
      updated_at = CURRENT_TIMESTAMP
  `;
}
