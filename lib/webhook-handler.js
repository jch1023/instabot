...
import { sendPrivateReply, renderDmMessage, getUserProfile } from './instagram.js';
...
export async function handleCommentEvent(event) {
    ...
    for (const campaign of campaigns) {
        ...
        if (campaign.check_follower) {
            // [단건 조회 로직] 무조건 API를 통해 실시간으로 확인
            console.log(`[Webhook] Verifying follower status for @${username} via API...`);

            try {
                const accessToken = await getAccessToken();
                if (accessToken) {
                    // [중요] 'is_user_follow_business' 필드 하나만 요청해야 'User consent' 에러를 피할 수 있을 수도 있음
                    const userProfile = await getUserProfile(accessToken, userId, 'is_user_follow_business');
                    console.log(`[DEBUG] Profile API Response for @${username}:`, JSON.stringify(userProfile));

                    // Meta Graph API: is_user_follow_business 필드 확인
                    if (userProfile.is_user_follow_business) {
                        isFollow = true;
                        console.log(`[Webhook] ✅ API Confirmed: @${username} IS following.`);
                    } else {
                        // 만약 API 필드가 아예 안 왔으면?
                        if (userProfile.is_user_follow_business === undefined) {
                            console.warn(`[Webhook] Warning: is_user_follow_business field is MISSING in API response. Check permissions.`);
                        }
                        isFollow = false;
                        console.log(`[Webhook] ❌ API Confirmed: @${username} IS NOT following.`);
                    }
                } else {
                ...
                console.error('[Webhook] Follower API check failed:', e.message);
                // API 실패 시 안전하게 비팔로우 처리
                isFollow = false;
            }
