...
import { getAllCampaigns, createDmLog, isFollower, getSetting, setSetting } from './db.js';
...
export async function handleCommentEvent(event) {
    ...
    const campaigns = getAllCampaigns().filter(c => c.is_active && c.execution_mode === 'webhook');
    const results = [];
    ...
    for (const campaign of campaigns) {
        ...
        try {
            ...
            setSetting(`processed_comment_${commentId}`, 'sent_via_webhook');
