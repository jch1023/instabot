...
function getCtaConfig(campaign, isFollow) {
    ...
    if (isFollow === true) {
        return {
            ...
            prompt: (campaign.cta_follower_prompt || '아래 버튼을 눌러 진행해주세요.').trim() || '아래 버튼을 눌러 진행해주세요.',
            legacyEnabled,
            legacyTitle,
            legacyPayload,
        };
    ...
    return {
        ...
        prompt: (campaign.cta_non_follower_prompt || '아래 버튼을 눌러 팔로우 상태를 다시 확인해주세요.').trim() || '아래 버튼을 눌러 팔로우 상태를 다시 확인해주세요.',
        legacyEnabled,
        legacyTitle,
        legacyPayload,
    };
...
async function sendFollowCtaMessage(accessToken, recipientId, ctaConfig) {
    ...
    const ctaPrompt = (ctaConfig.prompt || '').trim() || (isUrlCta
