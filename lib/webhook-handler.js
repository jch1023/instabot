...
export async function handleCommentEvent(event) {
    ...
    for (const campaign of campaigns) {
        ...
        if (campaign.check_follower) {
            ...
            // 2. 캐시에 없으면 -> API로 단건 조회 (is_user_follow_business)
            // 이것이 V21.0 API에서 지원하는 가장 확실한 방법입니다.
            if (!isFollow) {
                console.log(`[Webhook] User @${username} not in cache, checking single user profile API...`);
                try {
                    const accessToken = await getAccessToken();
                    if (accessToken) {
                        const userProfile = await getUserProfile(accessToken, userId);

                        // is_user_follow_business 필드 확인
                        if (userProfile.is_user_follow_business) {
                            console.log(`[Webhook] API confirmed @${username} IS following business!`);
                            isFollow = true;
                            // DB 캐시에 저장 (다음번엔 API 호출 안 하도록)
                            cacheFollowers(campaign.account_id || 1, [{ id: userId, username: username }]).catch(console.error);
                        } else {
                            console.log(`[Webhook] API confirmed @${username} is NOT following.`);
                        }
                    }
                } catch (e) {
                    console.error('[Webhook] Real-time follower check failed:', e.message);
                }
            }
