...
async function sendFollowCtaMessage(accessToken, recipientId, ctaConfig) {
    const isUrlCta = isWebUrl(ctaConfig.payload);
    const promptText = (ctaConfig.prompt || '').trim();
    const titleText = typeof ctaConfig.title === 'string' ? ctaConfig.title.trim() : '';
    const ctaTitle = titleText || '\u200B';
    const manualReplyKeyword = titleText || FOLLOW_RECHECK_TITLE;
    // Generic template requires a title. Use zero-width space when user wants "button only".
    const ctaPrompt = promptText || '\u200B';
    ...
    try {
        ...
        return {
            mode: isUrlCta ? 'web_url_template' : 'button_template',
        ...
        try {
            ...
            return { mode: 'quick_reply', templateType: null };
            ...
            return { mode: 'text_reply_fallback', templateType: null };
        }
    }
}
    ...
    for (const campaign of campaigns) {
        ...
        try {
            ...
            if (shouldAttachFollowCta) {
                ...
                const ctaSendResult = await sendFollowCtaMessage(accessToken, userId, ctaConfig);
                followCtaMode = ctaSendResult.mode;
                followCtaTemplateType = ctaSendResult.templateType;
            }
    ...
    try {
        ...
        for (const item of pending) {
            ...
            try {
                ...
                if (shouldAttachFollowCta) {
                    const ctaSendResult = await sendFollowCtaMessage(accessToken, senderId, ctaConfig);
                    followCtaMode = ctaSendResult.mode;
                    followCtaTemplateType = ctaSendResult.templateType;
                }
