...
function getCtaConfig(campaign, isFollow) {
    ...
    if (isFollow === true) {
        return {
            enabled: campaign.cta_follower_enabled === 1,
    ...
    return {
        enabled: campaign.cta_non_follower_enabled !== 0 && legacyEnabled,
...
export async function handleCommentEvent(event) {
    ...
    for (const campaign of campaigns) {
        ...
        const quickReplies = shouldAttachFollowCta
            ...
            : [];
        const outboundDmText = shouldAttachFollowCta
            ? `${dmText}\n\n아래 문구를 DM으로 답장해주세요:\n"${ctaConfig.title}"`
            : dmText;
        ...
        try {
            ...
            if (shouldAttachFollowCta) {
                ...
                // Official quick replies are sent through recipient.id (IGSID) messages.
                // If conversation window is closed, this can fail and we keep text-reply fallback.
                try {
                    ...
                    console.warn('[Webhook] CTA quick reply send skipped:', ctaError.message);
                }
            }
