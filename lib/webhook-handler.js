    ...
    for (const campaign of campaigns) {
        ...
        if (campaign.check_follower) {
            ...
            if (isFollow === true) {
                ...
                followerStatus = null;
                // Unknown state: use fallback opener text if configured.
        ...
        const ctaEnabled = campaign.cta_enabled !== 0;
        const ctaTitle = (campaign.cta_button_text || FOLLOW_RECHECK_TITLE).trim() || FOLLOW_RECHECK_TITLE;
        const ctaPayload = (campaign.cta_payload || FOLLOW_RECHECK_PAYLOAD).trim() || FOLLOW_RECHECK_PAYLOAD;
        const shouldAttachFollowCta = campaign.check_follower && isFollow !== true && ctaEnabled;
        ...
        try {
            ...
            if (quickReplies.length > 0) {
                await sendPrivateReplyWithOptions(accessToken, commentId, dmText, { quickReplies });
            ...
            results.push({
                ...
                dm_content: dmText,
                is_follower: followerStatus,
                follow_cta_attached: quickReplies.length > 0,
                status: 'success'
            });
...
export async function handleMessagingEvent(event) {
    ...
    try {
        ...
        const validPayloads = new Set(
            pending
                .map(p => (p.cta_payload || FOLLOW_RECHECK_PAYLOAD).trim())
                .filter(Boolean)
        );
        const validTitles = new Set(
            pending
                .map(p => (p.cta_button_text || FOLLOW_RECHECK_TITLE).trim())
                .filter(Boolean)
        );
