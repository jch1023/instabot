...
function composeInlineCtaMessage(baseMessage, ctaConfig) {
    ...
    if (isWebUrl(payloadText)) {
        ...
        lines.push(`답장: ${getCtaReplyTitle(ctaConfig)}`);
    }
...
export async function handleCommentEvent(event) {
    ...
    for (const campaign of campaigns) {
        ...
        const outboundDmText = shouldAttachFollowCta
            ? composeInlineCtaMessage(dmText, ctaConfig)
            : dmText;
        ...
        try {
            ...
            results.push({
                ...
                is_follower: followerStatus,
                follow_cta_mode: shouldAttachFollowCta ? 'inline_text' : 'none',
...
export async function handleMessagingEvent(event) {
    ...
    try {
        ...
        for (const item of pending) {
            ...
            const outboundMessage = shouldAttachFollowCta
                ? composeInlineCtaMessage(renderedMessage, ctaConfig)
                : renderedMessage;
