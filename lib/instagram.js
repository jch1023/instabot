...
/**
 * Reply to a comment (대댓글)
 * Endpoint: POST /{comment-id}/replies
 * Ref: https://developers.facebook.com/docs/instagram-platform/instagram-api-with-instagram-login/comment-moderation
 */
export async function replyToComment(accessToken, commentId, message) {
    ...
    if (!response.ok) {
        throw new Error(data.error?.message || `Failed to reply to comment (HTTP ${response.status})`);
    }
