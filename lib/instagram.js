...
export async function getUserProfile(accessToken, igUserId, fields = null) {
    // 기본 필드 (없으면 전체 조회)
    // 주의: 'name', 'profile_picture_url' 등은 타인 조회 시 'User consent' 에러 유발 가능성 있음
    const defaultFields = 'id,username,name,profile_picture_url,followers_count,media_count,is_user_follow_business';
    const fieldsToFetch = fields || defaultFields;

    const url = `${GRAPH_API_BASE}/${igUserId}?fields=${fieldsToFetch}&access_token=${accessToken}`;
