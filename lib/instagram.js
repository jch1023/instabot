...
export async function sendDirectButtonTemplate(accessToken, recipientId, text, button = {}, igUserId = 'me') {
    const url = `${GRAPH_API_BASE}/${igUserId}/messages`;
    const title = (button.title || '팔로우 했어요').trim() || '팔로우 했어요';
    ...
    const messageBody = isWebUrlButton
        ? {
            attachment: {
                type: 'template',
                payload: {
    ...
    if (!response.ok) {
        throw new Error(data.error?.message || `Failed to send button template (HTTP ${response.status})`);
    }
