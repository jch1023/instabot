...
export async function getUserProfile(accessToken, igUserId) {
    const fields = 'id,username,name,profile_picture_url,followers_count,media_count,is_user_follow_business,is_business_follow_user,follower_count';
    const url = `${GRAPH_API_BASE}/${igUserId}?fields=${fields}&access_token=${accessToken}`;

    const response = await fetch(url);
    const data = await response.json();

    if (!response.ok) {
        // API 에러 발생 시 로그를 남기지만, 로직 중단을 막기 위해 null 반환하거나 에러 처리
        console.error('[Instagram API] Failed to fetch profile:', data.error);
